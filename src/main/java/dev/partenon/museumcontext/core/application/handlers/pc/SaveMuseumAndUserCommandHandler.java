    ...
    public void handle(SaveMuseumAndUserCommand command) {
        Mono.just(command)
                .doOnNext(museum -> {
                    ...
                    if (Boolean.TRUE.equals(museumGhost.hasElement().block()))
                        throw new RuntimeException("Username ya se encuentra en uso");
                })
                .doOnNext(museum -> {
                    ...
                    if (Boolean.TRUE.equals(museumGhost.hasElement().block()))
                        throw new RuntimeException("Email ya se encuentra en uso");
                })
